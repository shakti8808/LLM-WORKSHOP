...
To maintain a clean and lightweight repository, this project utilizes a branch-based comparison workflow:

*   **`main`**: The base landing branch. It remains lightweight and contains only the root-level configuration (`.gitignore`) and this documentation (`README.md`). It does not store the application source code.
*   **`vibe_coded_submission`**: Contains the complete source code, tests, and documentation for the **vibe-coded** implementation.
*   **`sdd_submission`**: Contains the complete source code, tests, schemas, OpenSpec specifications, and documentation for the **Spec-Driven Development (SDD)** implementation.
